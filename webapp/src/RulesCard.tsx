...
export default function RulesCard({reportBack, dataType, selector, ruleKey, ruleValue}:{reportBack: (id: string, value: string) => void;dataType: string; selector: string; ruleKey: string; ruleValue: string}) {
    if (dataType === 'c'){
        ...
        return (<>
            <div className="h-9 ease-in-out flex transition-all group hover:h-27 duration-100 overflow-hidden bg-white bg-[conic-gradient(#ccc_25%,transparent_25%_50%,#ccc_50%_75%,transparent_75%)] bg-size-[18px_18px]">
                <div className="flex-col text-nowrap w-114.75 pt-1 shrink-0 border-[#000000] float-start bg-gray-800 text-gray-50">
                    <label className="flex-start m-1">
                        <span className="text-xs text-gray-300">color:&nbsp;</span>
                        <span className="font-bold">{ruleKey}</span>&nbsp;
                        <input value={ruleValue} className="w-40 focus:outline-1.5 outline-mist-100 bg-gray-900 rounded-md mr-1 hover:bg-[#171720] border-black pl-1 inset-shadow-md/40" onChange={
                            (e) => reportBack(ruleKey, e.target.value)}/> 
                    </label>
                    <code className="text-gray-300 text-xs font-mono">{fakeStyle.color}</code>
                    <div className="mt-1 bg-linear-to-r from-white to-(--hueColor)" style={{'--hueColor': `hsl(${colorHSL[0]} 100 50)`}as React.CSSProperties}>
                        <div className="bg-linear-to-t from-black to-[#0000]">
                            <div className="bg-white visible bg-[conic-gradient(transparent_25%,#ccc_25%_50%,transparent_50%_75%,#ccc_75%)] bg-size-[18px_18px] group-hover:invisible transition duration-100">
                                <div className="flex-auto h-19" style={{backgroundColor:cardColor}}/>
                            </div>
                        </div>
                    </div>
                </div>
